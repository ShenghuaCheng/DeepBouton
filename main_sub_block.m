...
imgTotal = cell(n, 1);
imgTIFF = ReadTiff(pathTIFF);
imgTIFFPad = padarray(imgTIFF, [50 50 15]);
swcArrayPad = swcArray;
swcArrayPad(:,3:5) = bsxfun(@plus, swcArrayPad(:,3:5), [50 50 15].*xyzRes);
for i = 1 : n % 
    ...
    [imgBlock, imgBlockSP, imgPrj, offset] = ReadBlockDataTIFF(imgTIFFPad, linexyz, r, binThre, eroThre, eroNum, swcArrayPad, xyzRes);
...
temp3 = bsxfun(@minus, temp3, [50 50 15]);
...
%WriteImageStack(imgTIFF, [pathSave, '\img.tif']);
